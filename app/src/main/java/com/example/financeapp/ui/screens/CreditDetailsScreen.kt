...
fun CreditDetailsScreen(
    ...
    FinanceScreenBackground(modifier = modifier.fillMaxSize()) {
        ...
        val allMarkedMessage = tr("Усі платежі вже відмічені", "All installments are already marked")
        ...
        Column(
                ...
                ElevatedCard(
                    ...
                    Column(
                        ...
                        if (isInstallmentPlan && totalInstallments > 0) {
                            Text(text = tr("Сплачено платежів: ${credit.paidInstallments} з $totalInstallments", "Paid installments: ${credit.paidInstallments} of $totalInstallments"))
            ...
            if (isInstallmentPlan) {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(
                        onClick = {
                            val canMark = totalInstallments == 0 || credit.paidInstallments < totalInstallments
                            if (canMark && credit.remainingAmount > 0.0) {
                                onMarkInstallmentPaid()
                                paymentError = null
                            } else {
                                paymentError = allMarkedMessage
                            }
                        },
                        modifier = Modifier.fillMaxWidth()
                    ) {
